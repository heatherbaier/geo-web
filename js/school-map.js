...
xhr.onreadystatechange = function() {
    if (this.readyState === 4 && this.status === 200) {
        ...
        console.log(response["query"][9], response["query"][10])

        // Create school Map
        var map = L.map('schoolMap').setView([response["query"][10], response["query"][9]], 16);

        var Esri_WorldImagery = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
            attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'
        }).addTo(map);

        var marker = L.marker([response["query"][10], response["query"][9]]).addTo(map);

        // Populate basic data card
        var schoolAddress = response["query"][4] ??= 'Not available.'
        var basicData = "<b>ADM1:</b> " + response["query"][6] + "<br><b>ADM2:</b> " + response["query"][7] + "<br><b>ADM3:</b> " + response["query"][8]
